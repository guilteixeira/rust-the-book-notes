...
Este capítulo do livro foi sensacional aprendemos um monte de recursos comuns em qualquer linguagem como declarar uma variável, capturar um input do usuário ou inserir comentários.
Porém muita coisa tem um jeitinho `Rust` de fazer, além disso, conhecemos melhor da sintaxe e do porque de alguns recursos que vimos no capítulo 1.
...
Outra coisa muito interessante foi o uso de referencia com o `&`, podendo ser uma referência a princípio imutável ou mutável, porém sem alterar o conteúdo de uma variável, o que torna tudo mais seguro, imagine isso como um ponteiro de `C` só que muito seguro. A sintaxe é bem simples podemos usar `&var` para uma referencia imutável, ou `&mut var` para uma referência mutável.

O recurso mais poderoso de uma referência é a capacidade de utilizar pedaços específicos de dados, sem a necessidade de os copiar constantemente para a memória. Isso torna nosso código mais rápido e seguro. No capítulo 4 o livro irá abordar referencias em mais detalhes.
...
Quando recebemos um input, seja qual for, é retornado um `result`, que pode ter a principio duas variações, `ok` quando operação ocorre com sucesso, ou `err` quando a operação falhou. A função `expect` é chamada sempre que `results` retorna `err`, portanto DEVE ser usada para tratar erros de forma elegante. `results` são como `structs` de `Go`, porém podem ter tipos diferentes, iremos abordar melhor `results` no capítulo 6.

O compilador te avisa, caso não utilize `expect` para tratar um erro. Você pode utilizar `expect` de forma bem simples passando uma mensagem de erro como argumento com `expect('Error...')`.
...
> O livro deixa claro que apesar de o `expect` ser uma boa tratativa, o mais indicado é tratarmos erros com códigos de erro, mas que isso será mostrado em detalhes no capítulo 9.
...
> Refazendo esse trecho do livro no dia 22/07/2025, a versão atual do `rand` é `0.9.2`, algumas coisas na sintaxe mudaram e irei atualizar isso abaixo.

Agora ao rodar `cargo build` novamente, vemos que além da `rand` todas as dependências de build também foram atualizadas e o `Cargo.toml` foi atualizado também, isso facilita demais a gestão de dependências, utilizando ferramentas de SAST (Static Application Security Test) e SCA (Software Composition Analisys) como `SonarQube`, `Veracode`, `Aikido` e `Github DependaBot`.
...
Após adicionar uma `crate` com o `cargo`, podemos utilizá-la no código, assim como fizemos com `std::io`. No jogo de adivinhação que criamos, usamos a `crate` `rand`, que vem de `random`(aleatório), para gerar um número aleatório dentro de um intervalo (`Range`).
...
Nesse trecho do capítulo, nosso código deveria ser:
...
fn main() {
    ...
    let secret_number = rand::thread_rng().gen_range(1..=100); // Implementação que o livro indica sobre o uso da Crate rand para gerar um número aleatório de 1 a 100.
...
No entanto, a função `thread_rng()` e a `trait` `.gen_range()` foram depreciadas e a sintaxe correta atualmente é `rng()` e `random_range()`. Dessa forma nosso código ficará, assim:
...
O Rust não para de surpreender, o `cargo` possui uma função muito bacana para documentar não só o nosso código mas todas as dependências e `crates` utilizadas no nosso projeto, para isso basta usar `cargo doc --open` e ele gera uma espécie de `swagger` localmente que fica salvo em `/target/doc/`.
...
Repare que `cmp` é um método de `guess`, que por sua vez é um inteiro recebido pelo usuário e que recebe como argumento, uma referência imutável (por padrão), do número que geramos randomicamente com a `crate rand`.
...
> O conceito de `shadowing` nesse ponto do livro ainda não foi muito esclarecido, mas o livro deixa claro que iremos falar disso depois. Até aqui, entenda que `shadowing` é basicamente atribuir um novo valor a uma variável já existente, esse valor pode ser do mesmo tipo ou de um tipo diferente para qual a variável foi criada a princípio.
...
> Após lê-lo novamente, percebi que ainda faltavam algumas anotações, agora acredito que deixei nítido todos os pontos mais relevantes e estou pronto para revisitar o capítulo 3.